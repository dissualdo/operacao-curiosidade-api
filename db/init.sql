...
CREATE TABLE `Authentication` (
    ...
    `Profile` longtext CHARACTER SET utf8mb4 NOT NULL,
...
CREATE TABLE `User` (
    ...
    `IdAuthentication` bigint NULL,
    CONSTRAINT `PK_User` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_User_Authentication_IdAuthentication` FOREIGN KEY (`IdAuthentication`) REFERENCES `Authentication` (`Id`)
) CHARACTER SET=utf8mb4;

CREATE TABLE `Curiosity` (
    `Id` bigint NOT NULL AUTO_INCREMENT,
    `UserId` bigint NOT NULL,
    `OtherInfo` varchar(2000) CHARACTER SET utf8mb4 NULL,
    `Interests` varchar(4000) CHARACTER SET utf8mb4 NULL,
    `Feelings` varchar(4000) CHARACTER SET utf8mb4 NULL,
    `CreatedAt` datetime(6) NOT NULL,
    `UpdatedAt` datetime(6) NULL,
    CONSTRAINT `PK_Curiosity` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_Curiosity_User_UserId` FOREIGN KEY (`UserId`) REFERENCES `User` (`Id`) ON DELETE CASCADE
) CHARACTER SET=utf8mb4;

CREATE UNIQUE INDEX `IX_Curiosity_UserId` ON `Curiosity` (`UserId`);

CREATE INDEX `IX_User_IdAuthentication` ON `User` (`IdAuthentication`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20250823005211_InitTable', '8.0.6');
...
INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20250823005309_InsertUserInitTable', '8.0.6');
